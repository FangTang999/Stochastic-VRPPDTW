...
int _tmain(int argc, TCHAR* argv[], TCHAR* envp[]) {
		...
		for (auto v : days) {
			auto minf = [&](double& minx, vector<ITEM>&path, int lp) {
				...
				cout 
					...
					<< " all_day_totalmincost" << low_up_bound(lp) << minx  << endl;
			};
	...
	cout << "Best Upper Bound:" << allmin
		<< " Best Lower Bound::"<<allminl<<endl;
	cout << "Gap between Best Upper Bound and Best Lower Bound: " << (allmin - allminl)/allmin * 100<<"%"<<endl;
	getchar();
	return 0;
}